function init() {
    ...
    let currentDataset = '../../data/choropleth/Adults.csv'; // Default dataset
    ...
    const zoom = d3.zoom()
        .scaleExtent([0.5, 8]) // Limit zoom scale
        .on("zoom", zoomed);

    // Apply zoom to the SVG element (not directly to the map paths)
    svg.call(zoom);

    // Zoom handler
    function zoomed(event) {
        g.attr("transform", event.transform); // Apply the zoom and pan transformations to the <g> element
    }

    function loadData(dataset) {
        Promise.all([
            d3.json('../../data/choropleth/world.geojson'),
            d3.csv(dataset)
    ...
    loadData(currentDataset); // Initial load with the default dataset (Both)
    ...
    d3.select("#both")
        .on("click", function() {
            ...
            d3.select("#female")
                .style('opacity', '0.75');
            currentDataset = '../../data/choropleth/Adults.csv'; // Change to Both dataset
    ...
    d3.select("#male")
        .on("click", function() {
            ...
            d3.select("#female")
                .style('opacity', '0.75');
            currentDataset = '../../data/choropleth/MaleAdults.csv'; // Change to Male dataset
            loadData(currentDataset);
        });
    ...
    d3.select("#female")
        .on("click", function() {
            ...
            d3.select("#female")
                .style('opacity', '1');
            currentDataset = '../../data/choropleth/FemaleAdults.csv'; // Change to Female dataset
            loadData(currentDataset);
        });
    ...
    function drawMap(year) {
        ...
        resetTooltip(); // Hide tooltip when drawing the map
    ...
    // Resize handling for responsive map
    window.addEventListener('resize', () => {
        ...
        drawMap(currentYear); // Use currentYear for redraw to maintain on the currently selected year
    });
}
